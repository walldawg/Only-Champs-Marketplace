...
echo "== Rookie Smoke Test v6 (Engine Core v1) =="
echo "BASE_URL=$BASE_URL"
echo
echo "== Ensure Wallet (test1) =="
curl -sS -X POST "$BASE_URL/wallet/activate" -H "x-user-id: test1" | jq .
echo
...
# If scoring returned an error, show it and fail.
if echo "$FINAL_JSON" | jq -e '.error?' >/dev/null 2>&1; then
  echo "Scoring failed:" >&2
  echo "$FINAL_JSON" | jq . >&2
  exit 1
fi
...
echo "== End Match =="
END_JSON="$(curl -sS -X POST "$BASE_URL/games/$GAME_ID/actions" -H 'content-type: application/json' -d '{"type":"ROOKIE_END_MATCH","payload":{}}' )"
if echo "$END_JSON" | jq -e '.error?' >/dev/null 2>&1; then
  echo "End match failed:" >&2
  echo "$END_JSON" | jq . >&2
  exit 1
fi

echo "$END_JSON" | jq '.state.rookie.phase, .state.rookie.rewardEligible'
echo

WIN_SEAT="$(echo "$END_JSON" | jq -r '.state.rookie.rewardEligible.winnerSeat')"
echo "WIN_SEAT=$WIN_SEAT"
echo

echo "== Claim Reward (as test1, seat=WIN_SEAT) =="
CLAIM_JSON="$(curl -sS -X POST "$BASE_URL/games/$GAME_ID/reward/claim" -H "x-user-id: test1" -H 'content-type: application/json' -d '{"seat":'"$WIN_SEAT"'}' )"
echo "$CLAIM_JSON" | jq .
echo

echo "== Wallet After Claim =="
curl -sS "$BASE_URL/wallet" -H "x-user-id: test1" | jq '.assets[] | select(.assetType=="EARNED")'
echo

echo "== Claim Reward Again (idempotent) =="
CLAIM2_JSON="$(curl -sS -X POST "$BASE_URL/games/$GAME_ID/reward/claim" -H "x-user-id: test1" -H 'content-type: application/json' -d '{"seat":'"$WIN_SEAT"'}' )"
echo "$CLAIM2_JSON" | jq .
echo
